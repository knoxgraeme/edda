...
export async function init(options: { nonInteractive?: boolean }) {
  ...
  // ── Step 8: Seed settings + apply wizard choices ────────────────────
  const s = p.spinner();
  ...
  try {
    ...
    await updateSettings({
      llm_provider: answers.llmProvider as import("@edda/db").LlmProvider,
      default_model: answers.llmModel,
      embedding_provider: answers.embeddingProvider as import("@edda/db").EmbeddingProvider,
    });
...
function buildEnvFile(a: WizardAnswers): string {
  const lines: string[] = [
    ...
    "# LLM provider and model are configured in the web UI settings page.",
    "",
    "# Database",
  ...
  if (a.llmApiKey && keyMap[a.llmProvider]) {
    lines.push("", "# LLM API Key", `${keyMap[a.llmProvider]}="${a.llmApiKey}"`);
  }
