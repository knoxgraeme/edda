...
│  - EddaOAuthProvider reads encrypted tokens from DB  │
...
export class EddaOAuthProvider implements OAuthClientProvider {
...
return {
  ...
  url,
  authProvider: new EddaOAuthProvider(connection.id, EDDA_BASE_URL),
};
```
...
// 2. If 401:
const provider = new EddaOAuthProvider(connection.id, EDDA_BASE_URL);
...
export async function GET(req: NextRequest) {
  ...
  const provider = new EddaOAuthProvider(connectionId, EDDA_BASE_URL);
...
The callback route lives in `apps/web` but needs to invalidate the MCP client singleton in `apps/server`. Options:

- **(a) Internal HTTP endpoint** — Add `POST /internal/mcp-oauth/complete` to `apps/server` that calls `invalidateMCPClient()`. The callback route calls this. Clean separation.
- **(b) Shared process assumption** — If both apps run in the same process, import directly. Document this assumption.

Recommend **(a)** for clean architecture. The endpoint is lightweight and internal-only.
...
| `apps/server/src/agent/mcp-oauth-provider.ts` | New — `EddaOAuthProvider` implementing `OAuthClientProvider` |
...
| `EddaOAuthProvider` | Each interface method with mocked DB. `tokens()` returns undefined when empty. `saveTokens()` with full `OAuthTokens`. `invalidateCredentials()` clears correct scope. |
